# -*- coding: utf-8 -*-
"""
Created on Fri Oct  9 04:17:06 2020
...
import mlrose
import numpy as np
weights = [10, 5, 2, 8, 15, 7,7,7,7]
values = [1, 2, 3, 4, 5, 6 , 7, 8, 9]
max_weight_pct = 0.6
fitness = mlrose_hiive.Knapsack(weights, values, max_weight_pct)
problem = mlrose_hiive.DiscreteOpt(length=9, fitness_fn=fitness, maximize=True, max_val=3)


algorithms_to_run = []#, 'SA', 'GA', 'MIMIC']

if 'RHC' in algorithms_to_run:
    # RHC
    print("Running RHC...")
    max_attempts = [200]
    restarts = [0, 100, 200, 400, 800, 1600]
    ...
    plt.savefig("RHC Parameter Tuning Fitness Curve Knapsack.png")
...
if 'SA' in algorithms_to_run:
    ...
    plt.grid(linewidth = 0.2)   
    plt.legend(loc="lower right",fontsize="small")
    plt.savefig("SA Parameter Tuning Fitness Curve Knapsack.png")

# # GA
if 'GA' in algorithms_to_run:
    print("Running GA...")
    #max_attempts = [10000]
    pop_size  = [100]#,200,300]# 1000, 1500]
   ...
   # from matplotlib import colormap
    ...
    plt.title("GA Parameter Tuning Fitness Curve, pop_size = 100")
    plt.xlim(-2,60)
    plt.xlabel('Iteration')
    plt.ylabel('Fitness Function Value')
    plt.grid(linewidth = 0.2)
    plt.legend(loc="lower right",fontsize="small")
    plt.savefig("GA Parameter Tuning Fitness Curve Knapsack.png")
...
if 'MIMIC' in algorithms_to_run:
    ...
    for i in pop_size:
        for j in keep_pct:
                ...
                starttime = timeit.default_timer()
                bs,bf,fc,rt = mlrose_hiive.mimic(problem=problem, max_attempts=500, pop_size = i, keep_pct= j, curve=True,random_state=seed,fast_mimic=True)#,state_fitness_callback=eval_times, callback_user_info=[3])
                endtime = timeit.default_timer()
    ...
    plt.grid(linewidth = 0.2)    
    plt.legend(loc="lower right",fontsize="small")
    plt.savefig("MIMIC Parameter Tuning Fitness Curve Knapsack.png")
...
for i in range(12):
    ...
    bs,bf,fc, rt_rhc = mlrose_hiive.random_hill_climb(problem=problem, max_iters = 500, max_attempts = 500, restarts = 800, curve=True,random_state=seed)#,state_fitness_callback=eval_times, callback_user_info=[3])
    fc_rhc.append(fc)
...
for i in range(12):
    seed = np.random.seed(i*10)
    bs,bf,fc, rt_sa = mlrose_hiive.simulated_annealing(problem=problem, max_iters = 500, max_attempts = 500,schedule=mlrose_hiive.ExpDecay(10,0.005), curve=True, random_state=seed)#,state_fitness_callback=eval_times, callback_user_info=[3])
    fc_sa.append(fc)
...
for i in range(12):
    seed = np.random.seed(i*10)
    bs, bf, fc, rt_ga = mlrose_hiive.genetic_alg(problem=problem, max_iters = 500, max_attempts = 500, pop_size = 1000, pop_breed_percent= 0.75, mutation_prob=0.7,curve=True, random_state=seed)#,state_fitness_callback=eval_times, callback_user_info=[3])
    fc_ga.append(fc)
fc_ga = tolerant_mean(fc_ga)

for i in range(12):
    seed = np.random.seed(i*10)
    bs,bf, fc, rt_mimic = mlrose_hiive.mimic(problem=problem, max_iters = 500, max_attempts = 500,pop_size = 80, keep_pct= 0.1, curve=True,random_state=seed,fast_mimic=True)#,state_fitness_callback=eval_times, callback_user_info=[3])
    fc_mimic.append(fc)
...
plt.subplot(1,2,1)
plt.title("Knapsack: Algorithm Comparisons Fitness Curve")
plt.xlabel('Iteration')
...
plt.plot(np.arange(1, len(fc_sa[0])+1), fc_sa[0], label="SA: s = ExpDecay, rate = 0.01, init_temp = 10", linewidth =1)
plt.fill_between(np.arange(1,len(fc_sa[0])+1), (fc_sa[0]-fc_sa[1]), (fc_sa[0] +fc_sa[1]), alpha=.1)
plt.plot(np.arange(1,len(fc_ga[0])+1), fc_ga[0], label="GA: ps = 100, pbp = 1, mp = 0.3", linewidth = 1)      
plt.fill_between(np.arange(1,len(fc_ga[0])+1), (fc_ga[0]-fc_ga[1]), (fc_ga[0] +fc_ga[1]), alpha=.1)
plt.plot(np.arange(1,len(fc_mimic[0])+1),fc_mimic[0],label="MIMIC: ps = 80, pct = 0.1", linewidth = 1)  
...
plt.subplot(1,2,2)
plt.title("Knapsack: Algorithm Comparisons Runtime Curve")
plt.xlabel('Iteration')

plt.plot(np.arange(1,len(rt_rhc)+1),rt_rhc,label="RHC: r = 800",linewidth = 1)
plt.plot(np.arange(1, len(rt_sa)+1), rt_sa, label="SA: s = ExpDecay, rate = 0.01, init_temp = 10", linewidth = 1)
plt.plot(np.arange(1,len(rt_ga)+1), rt_ga, label="GA: ps = 100, pbp = 1, mp = 0.3", linewidth = 1)      
plt.plot(np.arange(1,len(rt_mimic)+1),rt_mimic,label="MIMIC: ps = 80, pct = 0.1", linewidth = 1)  
...
plt.savefig("Algorithm Fitness and Runtime Curve Knapsack.png")
